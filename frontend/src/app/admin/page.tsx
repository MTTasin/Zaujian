  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        {/* Business-wide money, from the Finance cash-book — not per-order costing. */}
        <StatCard
          label="Net this month"
          value={`৳${Math.round(data.month_net).toLocaleString()}`}
          icon="wallet"
          tone={data.month_net >= 0 ? "green" : "amber"}
          hint={data.dues_total > 0
            ? `৳${Math.round(data.dues_total).toLocaleString()} unpaid credit`
            : undefined}
        />
      </div>
